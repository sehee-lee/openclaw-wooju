...
import { getTemplateFiles, type TemplateFiles } from "../commands/onboard-template.js";
...
export async function ensureAgentWorkspace(params?: {
  ...
  // Get standard template files
  const templateFiles: TemplateFiles = getTemplateFiles();
  ...
  const toolsContent = await loadTemplate(DEFAULT_TOOLS_FILENAME);
  const identityContent = await loadTemplate(DEFAULT_IDENTITY_FILENAME);
  const userContent = await loadTemplate(DEFAULT_USER_FILENAME);
  const heartbeatTemplate = await loadTemplate(DEFAULT_HEARTBEAT_FILENAME);
  const bootstrapTemplate = await loadTemplate(DEFAULT_BOOTSTRAP_FILENAME);

  await writeFileIfMissing(agentsPath, agentsContent);
  await writeFileIfMissing(soulPath, soulContent);
  await writeFileIfMissing(toolsPath, toolsContent);
  await writeFileIfMissing(identityPath, identityContent);
  await writeFileIfMissing(userPath, userContent);
