...
export async function promptDefaultModel(
  ...
  // Check for exact match or prefix match (e.g., "namc" matches "namc-oss", "namc-bedrock", etc.)
  const isProviderPrefixMatch = (provider: string) =>
    preferredProvider
      ? provider === preferredProvider || provider.startsWith(`${preferredProvider}-`)
      : false;

  const matchingProviders = providers.filter(isProviderPrefixMatch);
  const hasPreferredProvider = matchingProviders.length > 0;
  ...
  if (hasPreferredProvider && preferredProvider) {
    models = models.filter((entry) => isProviderPrefixMatch(entry.provider));
  }
