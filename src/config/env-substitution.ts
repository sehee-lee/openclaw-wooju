      ...
      if (end !== -1) {
        ...
        if (ENV_VAR_NAME_PATTERN.test(name)) {
          ...
          if (envValue === undefined || envValue === "") {
            if (allowMissing) {
              // Keep original ${VAR} string if missing and allowed
              chunks.push(`\${${name}}`);
              i = end;
              continue;
            }
 ...
 * @param options.allowMissing - If true, missing env vars are preserved as `${VAR}` instead of throwing
 * @returns The config object with env vars substituted
 * @throws {MissingEnvVarError} If a referenced env var is not set or empty (unless allowMissing is true)
 */
