...
export type ConfigIoDeps = {
  ...
  /** If true, missing env vars in ${VAR} references are preserved as-is instead of throwing */
  allowMissingEnvVars?: boolean;
};
...
function normalizeDeps(overrides: ConfigIoDeps = {}): Required<ConfigIoDeps> {
  return {
    ...
    allowMissingEnvVars: overrides.allowMissingEnvVars ?? false,
  };
}
  ...
  function loadConfig(): OpenClawConfig {
    try {
      ...
      const substituted = resolveConfigEnvVars(resolved, deps.env, {
        allowMissing: deps.allowMissingEnvVars,
      });
      ...
      // Silently return empty config for missing env vars (common during onboarding
      // when API keys haven't been entered yet)
...
export function loadConfig(options?: { allowMissingEnvVars?: boolean }): OpenClawConfig {
  const io = createConfigIO({ allowMissingEnvVars: options?.allowMissingEnvVars });
...
export async function readConfigFileSnapshot(options?: {
  ...
  return await createConfigIO({
    allowMissingEnvVars: options?.allowMissingEnvVars,
  }).readConfigFileSnapshot();
}
